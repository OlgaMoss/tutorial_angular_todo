...
var limit_formvalidation_1 = require("./limit.formvalidation");
            ...
            for (var errorName in this.errors) {
                switch (errorName) {
                    ...
                    case "minlength":
                        ...
                        break;
                    case "maxlength":
                        messages.push("A " + this.label + " cannot be more than " + this.errors['maxlength'].requiredLength + " characters");
                        break;
                    case "limit":
                        messages.push("A " + this.label + " must be no more than " + this.errors['limit'].limit);
                        break;
                    case "pattern":
...
var ProductFormGroup = (function (_super) {
    ...
    function ProductFormGroup() {
        _super.call(this, {
            ...
            category: new ProductFormControl('Category', 'category', '', forms_1.Validators.compose([forms_1.Validators.required, forms_1.Validators.pattern("^[A-Za-z ]+$"), forms_1.Validators.minLength(3), forms_1.Validators.maxLength(10)])),
            price: new ProductFormControl('Price', 'price', '', forms_1.Validators.compose([forms_1.Validators.required, limit_formvalidation_1.LimitValidator.Limit(100), forms_1.Validators.pattern("^[0-9\.]+$")]))
        });
    }
    ...
    ProductFormGroup.prototype.getFormValidationMessages = function (form) {
        var messages = [];
        this.productControls.forEach(function (c) {
            c.getValidationMessages().forEach(function (m) { return messages.push(m); });
        });
        return messages;
    };
